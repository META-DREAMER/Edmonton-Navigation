...
//initialize global pointers to waypoints and path length
LonLat32* waypointsptr;
int* path_lenptr;

//First path flag to ensure path is only drawn after a path has been found
...
void loop() {
        ...
        if (request_state==RS_WAIT_FOR_START) {
            ...
            if (path_len>0) {
                ...
                if (srv_get_waypoints(waypoints,path_len)>=0) {
                    ...
                    debug_msg("Data not received.");
                }
                waypointsptr = waypoints; //set pointers to waypoints and pathlen
                path_lenptr = &path_len;
                firstpath = true; //set flag to true after first path found

                draw_map_screen(); //Draw the path
                draw_cursor();
                draw_path();
            } else {
    ...
    if (update_display_window) {
        ...
        //draw the path only if a path was found already
        if (firstpath) {
            draw_path();
        }
