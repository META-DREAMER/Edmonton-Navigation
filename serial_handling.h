...
/*
  ...
  appended onto the string) or until timeout.  This function is blocking. 
  ...
  timeout: The time in milliseconds to wait for data until timeout

  Preconditions:  None.

  Postconditions: Function will block until a full newline has been read, or the
    maximum length has been reached, or timeout reached.  Afterwards the new 
    string will be stored in the buffer passed to the function.

  Returns: 1 if timeout, 0 if bytes read succesfully.
